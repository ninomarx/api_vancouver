...
var CourseBusiness = (function() {
    ...
    CourseBusiness.prototype.select = function(courseModel, callback) {
        ...
        sql = sql + " US.use_image, CL.cla_allow_lateregistration,cit_time_zone, ";
        ...
        sql = sql + "    WHERE cor_id = COU.cor_id and clt_date >= "+time_zone_date+" and cla_status = 'A' AND clt_firstClass = 'Y' order by clt_date,clat.clt_start_time limit 1) as date_first_class ";
        ...
        if(courseModel.use_id != "")
            sql = sql + "   LEFT JOIN wishlist WS ON CL.cor_id = WS.cor_id AND WS.use_id = " + courseModel.use_id + " ";
    ...
    CourseBusiness.prototype.selectCourseTeaching = function(courseModel, callback) {
        ...
        sql = sql + " (select count(wis_id) from wishlist where cor_id = COU.cor_id) wishlist, ";
    ...
    CourseBusiness.prototype.selectByFilter = function(courseModel, callback) {
        ...
        if(courseModel.filter.begin_date != "" && courseModel.filter.end_date != "" ){
            ...
            sql = sql + " AND clt_date >= "+time_zone_date+" ";
        }
        ...
        sql = sql + "  order by clt_date,clat.clt_start_time limit 1) as date_first_class ";
        ...
        sql = sql + "      ct.clt_date >= DATE_ADD("+time_zone_date+", INTERVAL -1 DAY) ";
    ...
    CourseBusiness.prototype.selectBySearch = function(courseModel, callback) {
        ...
        if(courseModel.search != "") {
            ...
            //sql = sql + " US.use_first_name like \"%" + courseModel.search + "%\" OR US.use_last_name like \"%" + courseModel.search + "%\" OR ";

            var str = courseModel.search;
            var res = str.split(" ");
            var search_text = "";
            ...
            sql = sql + " OR COU.cor_id in (select cor_id from course_tags where cta_tag REGEXP '" + search_text + "' )) ";
        }
    ...
    CourseBusiness.prototype.selectInactiveCourse = function(courseModel, callback) {
        ...
        sql = sql + " LEFT JOIN course_subcategory CSU ON COU.cor_id = CSU.cor_id ";
        if(courseModel.use_id != "")
            sql = sql + " LEFT JOIN wishlist WS ON COU.cor_id = WS.cor_id AND WS.use_id = " + courseModel.use_id + " ";
        sql = sql + " WHERE COU.cor_status = 'A' ";
        sql = sql + " AND CL.cla_status = 'A' ";
        sql = sql + " AND US.use_type = 2 ";
        sql = sql + " AND ct.clt_date < Date_add("+time_zone_date+", INTERVAL -1 day) ";
